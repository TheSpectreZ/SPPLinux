...
layout(set = 1, binding = 1) buffer DrawVisibility
{
	uint drawVisibility[];
};

layout(set = 1, binding = 2) uniform sampler2D depthPyramid;
...
void main()
{
	...
	//vec2 ViewScalar = vec2(cullData.pyramidWidth,  cullData.pyramidHeight) / vec2( ViewConstants.ViewFrame.zw );
	uint renderIdx = di;
