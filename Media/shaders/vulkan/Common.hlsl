...
#else
float4x4 GetLocalToWorldViewTranslated(in float4x4 InLTWSR, in float3 InTrans, in float3 InViewPosition)
#endif
{
	return float4x4(
...
#else
float4x4 GetWorldToLocalViewTranslated(in float4x4 InLTWSR, in float3 InTrans, in float3 InViewPosition)
#endif
{
