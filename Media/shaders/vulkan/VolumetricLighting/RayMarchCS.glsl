...
layout( set = 1, binding = 1) uniform sampler3D s_InjectionResult[ 2 ];
layout( set = 1, binding = 2, rgba16f) uniform writeonly image3D i_RayMarchOutput; 
...
{
    float Near = VolumeData.nearZ;
    float Far = VolumeData.farZ;
...
void main()
{
    ...
    for(int z = 0; z < VOXEL_GRID_SIZE_Z; z++)
    {
        ...
        vec4 slice_scattering_density = texelFetch(s_InjectionResult[ PushConstant.injectionImageIndex ], coord, 0);
        ...
        imageStore(i_RayMarchOutput, coord, accum_scattering_transmittance);
    }
}