...
layout(set = 0, binding = 2) uniform sampler2D windNoiseTexture;

layout(set = 0, binding = 3) readonly uniform _WindParams
{		
	vec3 direction;
	vec2 scollOffset;
	float posScale;	
...
void main()
{
	...
	// lazyness
	float windValue = texture(windNoiseTexture, (inPos.xy + inPos.zz) * WindParams.posScale + WindParams.scollOffset ).r * WindParams.intensity;
	float vertScalar = inUV[1].r;

	vec3 localPos = inPos + (WindParams.direction * windValue * vertScalar);

	vec4 finalPos = Multiply( vec4(localPos, 1.0), localToScreen );
