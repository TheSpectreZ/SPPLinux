...
#include "SDFCommonShapes.glsl"
#include "SDFOperators.glsl"
...
layout(set = 0, binding = 2) readonly uniform _DrawParams
{
	vec3 ShapeColor;
    uint ShapeCount;
} DrawParams;

layout(set = 0, binding = 3) readonly uniform _Shapes
{
	mat4 invTransform;
...
bool intersect_ray_sphere(vec3 rayOrigin, vec3 rayDirection, vec3 sphereCenter, float sphereRadius, out float t0, out float t1)
{         
...
groupshared mat4 ViewToShapeMatrices[MAX_SHAPES];
...
void ShapeGenerateShared()
{
	...
	{
		ViewToShapeMatrices[ShapeIdx] = Mulitply(GetTranslationMatrix(vec3(ViewConstants.ViewPosition - DrawConstants.Translation)), Shapes[ShapeIdx].invTransform);
	}
}
...
uint shapeCullAndProcess(in vec3 ro, in vec3 rd, out float T0, out float T1)
{
    ...
    for (uint ShapeIdx = 0; ShapeIdx < DrawParams.ShapeCount && ShapeIdx < MAX_SHAPES; ++ShapeIdx)
    {
		vec3 rayStart = Multiply(vec4(ro, 1.0), ViewToShapeMatrices[ShapeIdx]).xyz;
		vec3 rayUnitEnd = Multiply(vec4(ro + rd, 1.0), ViewToShapeMatrices[ShapeIdx]).xyz;
		shapeProcessedData[ShapeIdx].rayScalar = 1.0f / length(rayStart - rayUnitEnd);
		vec3 rayDirection = normalize(rayStart - rayUnitEnd);
				
        shapeProcessedData[ShapeIdx].IsHit = intersect_ray_sphere(rayStart, rayDirection, vec3(0,0,0), 2.0f, shapeProcessedData[ShapeIdx].T0, shapeProcessedData[ShapeIdx].T1);
...
float processShapes( in vec3 pos, out vec3 hitColor )
{
    ...
    for (uint i = 0; i < DrawParams.ShapeCount && i < MAX_SHAPES; ++i)
    {
		...
		if (shapeProcessedData[i].IsHit)
		{
			...
			vec3 samplePos = Multiply(vec4(pos, 1.0), ViewToShapeMatrices[i]).xyz;
...
vec4 renderSDF( in vec3 ro, in vec3 rd ) 
{ 
	...
	return vec4(rd * 0.5 + 0.5, hitDistance);
}
