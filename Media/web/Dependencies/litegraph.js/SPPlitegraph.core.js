...
*/
    ...
    */
    ...
    LGraphNode.prototype.addWidget = function( type, name, value, callback, options )
	{
        ...
        if(w.options.x !== undefined) {
            w.x = w.options.x;
        }

        if(w.options.width !== undefined) {
            w.width = w.options.width;
        }
        ...
        for (var i = 0; i < widgets.length; ++i) {
            ...
            switch (w.type) {
                case "button":
                    ...
                    ctx.fillRect(x, y, widget_width - margin * 2, H);
					if(show_text && !w.disabled)
	                    ctx.strokeRect( x, y, widget_width - margin * 2, H );
                    if (show_text) {
                        ctx.textAlign = "center";
                        ctx.fillStyle = text_color;
                        ctx.fillText(w.label || w.name, x + widget_width * 0.35, y + H * 0.7);
                    }
                    break;
                case "toggle":
                    ctx.textAlign = "left";
                ...
                case "combo":
                    ...
                    ctx.beginPath();
					if(show_text)
	                    ctx.roundRect(x, y, widget_width, H, [H * 0.5] );
					else
	                    ctx.rect(x, y, widget_width, H );
                    ctx.fill();
                    if (show_text) {
						if(!w.disabled)
		                    ctx.stroke();
						...
						if(!w.disabled)
						{
							...
							ctx.lineTo(x + 16, y + H - 5);
							ctx.fill();
							ctx.beginPath();
							ctx.moveTo(x + widget_width - margin * 0.5 - 9, y + 5);
							ctx.lineTo(x + widget_width - margin * 0.5 , y + H * 0.5);
							ctx.lineTo(x + widget_width - margin * 0.5 - 9, y + H - 5);
							ctx.fill();
						}
                        ctx.fillStyle = secondary_text_color;
                        ctx.fillText(w.label || w.name, x + margin * 1.2 + 5, y + H * 0.7);
                        ctx.fillStyle = text_color;
                        ctx.textAlign = "right";
                        if (w.type == "number") {
                            ctx.fillText(
    ...
    LGraphCanvas.prototype.processNodeWidgets = function(
        ...
        for (var i = 0; i < node.widgets.length; ++i) {
			...
			if ( w != active_widget) {
                ...
                var bound = widget_width;
                if(w.x) 
                    bound += w.x;

                if (x < w.x || x > bound)
                    continue;
                if(y < w.last_y || y > w.last_y + widget_height || w.last_y === undefined) 
                    continue;
            }
			...
			switch (w.type) {
				...
				case "combo":
					...
					if (event.type == LiteGraph.pointerevents_method+"move" && w.type == "number") {
                        ...
                        var dir = x < 40 ? -1 : x > widget_width - 40 ? 1 : 0;
                        var delta = dir * (event.deltaX ? event.deltaX : 0.1);
                        ...
                        if(w.x)
                            delta = x < (w.x + 40) ? -1 : x > (widget_width + w.x - 40) ? 1 : 0
                        else
        ...
        function inner_onMenuAdded(base_category ,prev_menu){
            ...
            nodes.map(function(node){
                ...
                var entry = { value: node.type, content: node.title, has_submenu: false , callback : function(value, event, mouseEvent, contextMenu){
                        ...
                        var node = LiteGraph.createNode(value.value, null, contextMenu.options.SPPConfig);
                        if (node) {
            ...
            new LiteGraph.ContextMenu( entries, { event: e, parentMenu: prev_menu, SPPConfig: canvas.graph.SPPConfig  }, ref_window );
